...
# Wrapper script to unlink custom node from a local n8n server
# Uses the main unlink_node_from_server.sh script from the central scripts directory
...
if [ ! -f "$MAIN_SCRIPTS_DIR/unlink_node_from_server.sh" ]; then
    echo "Error: Main unlink script not found at $MAIN_SCRIPTS_DIR/unlink_node_from_server.sh"
    exit 1
fi
...
if [ -z "$1" ]; then
    ...
    if [ "$selected" -eq $((server_count+1)) ]; then
        ...
        for dir in "${server_dirs[@]}"; do
            echo "Unlinking from server: $(basename "$dir")"
            "$MAIN_SCRIPTS_DIR/unlink_node_from_server.sh" "$CUSTOM_NODE_DIR" "$dir"
            echo "---------------------------------------------"
        done
        exit 0
    else
...
"$MAIN_SCRIPTS_DIR/unlink_node_from_server.sh" "$CUSTOM_NODE_DIR" "$SERVER_PATH"